...
let test_flat_at board x y =
  let rest = ['X'; 'M'; 'A'; 'S'] in
  List.map (fun (dx, dy) -> List.mapi (fun i c -> char_at board (x + i * dx) (y + i * dy) = Some(c)) rest |> all |> int_of_bool) dirs |> List.fold_left (+) 0
...
let test_pattern_at board x y =
  let rest = ['M'; 'A'; 'S'] in
  let all_dirs = pairs dirs |> List.filter (fun p -> not (is_same_dir p) && all_diag p) in
  let test_c dx dy i c = char_at board (x + (i - 1) * dx) (y + (i - 1) * dy) = Some(c) in
  let test_pat ((dx1, dy1),(dx2, dy2)) = [List.mapi (fun i c -> test_c dx1 dy1 i c) rest |> all; List.mapi (fun i c -> test_c dx2 dy2 i c) rest |> all] |> all in
...
let runPart1 b =
  ...
  List.map (fun y -> List.map (fun x -> test_flat_at b x y) for_x) for_y |> List.flatten |> List.fold_left (+) 0
  
let runPart2 b =
  let for_y = List.init b.height (fun i -> i) in
  let for_x = List.init b.width (fun i -> i) in
  List.map (fun y -> List.map (fun x -> test_pattern_at b x y) for_x) for_y |> List.flatten |> List.fold_left (+) 0
