...
module Input = struct
  ...
  let of_string s =
    ...
    { rules = rules |> Ordering.of_list; pages = List.map page_of_string pages }
end
...
let is_valid (input : Input.t) page =
  let rec is_valid' page =
    ...
    | [ _ ] -> true
    | x :: xs ->
        ListExt.all_predicate (fun y -> Ordering.mem (x, y) input.rules) xs
        && is_valid' xs
  in
  is_valid' page
...
let topological_sort (input : Input.t) page =
  let subgraph =
    input.rules
    |> Ordering.filter (fun (a, m) -> List.mem a page && List.mem m page)
  in
...
let runPart1 input =
  List.filter (is_valid input) input.pages
  |> List.map select_middle |> List.fold_left ( + ) 0

let runPart2 (input : Input.t) =
  List.filter (fun x -> not (is_valid input x)) input.pages
  |> List.map (topological_sort input)
  |> List.map select_middle |> List.fold_left ( + ) 0
