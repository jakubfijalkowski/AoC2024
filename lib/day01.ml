...
let runPart1 first second = List.fold_left2 (fun acc a b -> acc + Int.abs (a - b)) 0 first second

let runPart2 first second = List.fold_left (fun acc a -> acc + a * (List.length (List.filter (fun b -> a = b) second))) 0 first
