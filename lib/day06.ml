...
let next_state m s =
  ...
  if Map.is_in_map m new_pos then
    if Map.is_empty m new_pos then Some { pos = new_pos; dir = s.dir }
    else Some { pos = s.pos; dir = new_dir }
  else None
...
let runPart1 m =
  ...
  PositionSet.cardinal
    (simulate
       (PositionSet.singleton (Map.get_starting_pos m))
       { pos = Map.get_starting_pos m; dir = North })
...
let runPart2 map =
  ...
  let rec simulate m visited s i =
    match next_state m s with
    | Some s' ->
        if StateSet.mem s' visited then 1
        else simulate m (StateSet.add s' visited) s' (i + 1)
    | None -> 0
  in
  let try_single (x, y) =
  ...
  in
  let res =
    Seq.init map.height (fun y -> Seq.init map.width (fun x -> (x, y)))
    |> Seq.concat |> Seq.map try_single |> Seq.fold_left ( + ) 0
  in
  res
