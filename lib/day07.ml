...
module Input =
  struct
    ...
    let of_string s =
      ...
      List.map (fun line ->
        let (sum, ops) = match String.split_on_char ':' line with
          | [sum; ops] -> (int_of_string sum, ops)
          | _ -> failwith "Invalid input" in
        let nums = String.split_on_char ' ' ops |> List.filter (fun x -> x <> "") |> List.map (fun i -> String.trim i |> int_of_string) in
        (sum, nums)
      ) lines
  end
...
let can_evaluate_simple (sum, nums) =
  let rec try_evaluate partial rest =
    match rest with
    | [] -> partial = sum
    | h::t -> try_evaluate (partial + h) t || try_evaluate (partial * h) t in
...
let can_evaluate_complex (sum, nums) =
  let rec try_evaluate partial rest =
    match rest with
    | [] -> partial = sum
    | h::t -> try_evaluate (partial + h) t || try_evaluate (partial * h) t || try_evaluate (concat_num partial h) t in
  try_evaluate (List.hd nums) (List.tl nums)
