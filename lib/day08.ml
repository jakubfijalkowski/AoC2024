...
module Input = struct
  ...
  let of_string s =
    ...
    let antennas =
      Seq.init height (fun y -> Seq.init width (fun x -> (x, y)))
  ...
  let is_inside { width; height; _ } (x, y) =
...
let try_add_all_antinodes input set a b =
  ...
  let dir1 =
    Seq.fold_left
      (fun acc m -> try_add_single_antinode m input acc a b)
      set muls
  in
  let dir2 =
    Seq.fold_left
      (fun acc m -> try_add_single_antinode m input acc b a)
      dir1 muls
  in
  dir2
...
let generate_antinodes generator (input : Input.t) =
  let rec add_all_single acc x = function
    | [] -> acc
    | y :: ys -> add_all_single (generator input acc x y) x ys
  in
  let rec add_all acc = function
    | [] -> acc
    | x :: xs -> add_all (add_all_single acc x xs) xs
  in
  Antennas.fold
    (fun _ lst acc -> add_all acc lst)
    input.antennas AntinodesSet.empty
